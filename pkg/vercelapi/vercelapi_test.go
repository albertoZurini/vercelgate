...
func TestGetTeams_Success(t *testing.T) {
	defer serve(t, func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(GetTeamsResponse{
			Teams: []Team{
				{ID: "t_1", Name: "Acme", Slug: "acme"},
				{ID: "t_2", Name: "Beta", Slug: "beta"},
			},
		})
	})()

	teams, err := GetTeams("tok_test")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(teams) != 2 {
	...
	if teams[0].ID != "t_1" || teams[1].Slug != "beta" {
		t.Errorf("unexpected teams: %+v", teams)
	}
}

func TestGetTeams_EmptyToken(t *testing.T) {
	_, err := GetTeams("")
	if err == nil {
		t.Error("expected error for empty token")
	}
...
func TestGetTeams_APIError(t *testing.T) {
	defer serve(t, func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(GetTeamsResponse{
			Error: Error{Code: "forbidden", Message: "invalid token"},
		})
...
func TestGetTeams_Empty(t *testing.T) {
	defer serve(t, func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(GetTeamsResponse{Teams: []Team{}})
	})()
	...
	if len(teams) != 0 {
		t.Errorf("expected empty teams list, got %d", len(teams))
	}
}
